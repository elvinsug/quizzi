...
<body>
    <div class="quizzi-bg" id="gameScreen" style="padding:2rem;">

        <!-- Question View -->
        <div id="viewQuestion" class="hidden" style="width:100%; max-width:960px;">
            ...
            <div class="text-center mt-3">
                <button class="btn btn-purple btn-lg" onclick="showResults()">Show Results</button>
            </div>
        </div>

        <!-- Results View -->
        <div id="viewResults" class="hidden" style="width:100%; max-width:960px;">
            <div class="question-header">
                <span class="question-number" id="rQNumber"></span>
            ...
            <p class="text-center mt-2" style="font-size:1.1rem; opacity:0.85;" id="rCorrectPercent"></p>
            <div class="text-center mt-3">
                <button class="btn btn-purple btn-lg" onclick="showLeaderboard()">Show Leaderboard</button>
            </div>
        </div>

        <!-- Leaderboard View -->
        <div id="viewLeaderboard" class="hidden" style="width:100%; max-width:600px;">
            <h1 class="text-center" style="font-size:2rem; margin-bottom:1.5rem;">&#127942; LEADERBOARD</h1>
        ...
        <div id="viewPodium" class="hidden" style="width:100%; max-width:700px;">
            <h1 class="text-center" style="font-size:2rem; margin-bottom:1rem;">&#127942; FINAL RESULTS &#127942;</h1>
    ...
    <script>
        ...
        function showResultsView(data) {
            ...
            document.getElementById('rCorrectPercent').textContent =
                correctCount + ' / ' + total + ' got it right (' + pct + '%)';

            if (resultsChart) resultsChart.destroy();
            const ctx = document.getElementById('resultsChart').getContext('2d');
            const barColors = ['#E21B3C','#1368CE','#D89E00','#26890C'];
            const borderColors = [
                data.correctAnswer === 'a' ? '#27AE60' : 'transparent',
                data.correctAnswer === 'b' ? '#27AE60' : 'transparent',
                data.correctAnswer === 'c' ? '#27AE60' : 'transparent',
                data.correctAnswer === 'd' ? '#27AE60' : 'transparent'
            ];
            resultsChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [data.countA||0, data.countB||0, data.countC||0, data.countD||0],
                        backgroundColor: barColors,
                        borderColor: borderColors,
                        borderWidth: borderColors.map(c => c === 'transparent' ? 0 : 4),
                        borderRadius: 8
                    }]
                },
                options: {
        ...
        function showLeaderboardView(data) {
            ...
            if (isLast) {
                actions.innerHTML = '<button class="btn btn-green btn-lg" onclick="finishGame()">Finish Game</button>';
            } else {
                actions.innerHTML = '<button class="btn btn-purple btn-lg" onclick="nextQuestion()">Next Question</button>';
            }
        }

        async function nextQuestion() {
            await fetch('/quizzi/api/next?sessionId=' + sessionId + '&action=next_question', { method: 'POST' });
        }

        async function finishGame() {
            await fetch('/quizzi/api/next?sessionId=' + sessionId + '&action=finish', { method: 'POST' });
        }
        ...
        function showPodiumView(data) {
            ...
            // Render podium in 2nd-1st-3rd order
            const order = [1, 0, 2];
            podium.innerHTML = '';
            order.forEach(idx => {
                if (lb[idx]) {
